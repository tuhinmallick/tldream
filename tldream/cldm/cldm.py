...
from tldream.ldm.modules.diffusionmodules.openaimodel import (
    UNetModel,
...
from tldream.ldm.util import log_txt_as_img, exists, instantiate_from_config
...
class ControlLDM(LatentDiffusion):
    ...
    def load_control_diff(self, diff_state_dict):
        ...
        final_state_dict = {}
        counter = 0
        for key in diff_state_dict.keys():
            if not key.startswith("control_model."):
                continue

            p = diff_state_dict[key]
        ...
        logger.info(f"Transfer {counter} control parameters")
